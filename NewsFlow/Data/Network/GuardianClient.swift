...
final class GuardianClient: GuardianClientProtocol {
    ...
    func search(query: String?, page: Int, pageSize: Int, section: String?) async throws -> [Article] {
        ...
        guard let url = components.url else {
            NewsFlowLogger.shared.error("Invalid Guardian URL", category: "Network")
            throw NewsAPIError.invalidURL
        }
        ...
        do {
            ...
            if let httpResponse = response as? HTTPURLResponse {
                NewsFlowLogger.shared.debug("Guardian API status \(httpResponse.statusCode)", category: "Network")
            }
            ...
            NewsFlowLogger.shared.error("Guardian API error: \(error.debugDescription)", category: "Network")
            throw error
        } catch {
            NewsFlowLogger.shared.error("Guardian request failed: \(error.localizedDescription)", category: "Network")
            throw error
        }
    }
}
